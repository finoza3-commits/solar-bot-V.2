...
def _build_status_reply(readings: dict | None, current_time: str) -> str:
    ...
    return (
        ...
        f"----------\n"
        f"📈 *ยอดสะสมของวันนี้*\n"
        f"🏠 บ้านใช้รวม: {home_daily_kwh:,.2f} kWh ({home_daily_kwh * COST_PER_UNIT:,.2f} ฿)\n"
        f"☀️ โซลาร์ผลิตได้: {solar_daily_kwh:,.2f} kWh ({solar_saved:,.2f} ฿)\n"
        f"⚡️ ไฟฟ้าที่ต้องซื้อ: {grid_actual_kwh:,.2f} kWh ({grid_cost:,.2f} ฿)\n"
        ...
        f"({readings.get('total_production', 0) * COST_PER_UNIT:,.2f} ฿)\n"
